...
class FSB5:
    ...
    def rebuild_sample(self, sample):
        ...
        if self.header.mode == SoundFormat.MPEG:
            ...
            from .vorbis import rebuild as vorbis_rebuild
            return vorbis_rebuild(sample)
        elif self.header.mode.is_pcm:
            from .pcm import rebuild as pcm_rebuild, rebuild_float as float_rebuild

            if self.header.mode == SoundFormat.PCM8:
                return pcm_rebuild(sample, 1)
            elif self.header.mode == SoundFormat.PCM16:
                return pcm_rebuild(sample, 2)
            elif self.header.mode == SoundFormat.PCM32:
